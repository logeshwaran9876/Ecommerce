...
?>
<h2 class="text-center m-3 bg-secondary ">WishList Items</h2>
<script>
...
<script>
        ...
        success: function(data) {
            ...
            if (Array.isArray(data) && data.length > 0) { 
                data.forEach(item => {
                  $('#contain').append(`
                          <div class="col">
                              <div class="card position-relative" style="width: 17rem; height:400px">
                                  <button type="button" id="clo" class=" position-absolute" aria-label="Close" style=" right: 2px; z-index: 1; font-size: 1.5rem; padding: 5px 5px;" data-product="${item.product_id}">
            ...
            `);
                ...
                $('#contain').html('<p>No products found in the cart.</p>');
            }
        },
        error: function(jqXHR) {
  ...
  loadwish();
    ...
    $('#contain').on('click', '#clo', function() {

      const productId = $(this).data('product');
    const customerId = <?php echo $_SESSION['user_id']; ?>; 
